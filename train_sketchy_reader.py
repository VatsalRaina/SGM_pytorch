...
from models_transformer import SketchyReader
...
parser.add_argument('--num_encoder_layers', type=int, default=6, help='Number of layers of transformer encoder')
parser.add_argument('--dim_feedforward', type=int, default=2048, help='Dimensions in transformer feedforward network')
...
def _join_pr_resp(p, p_len, r, r_len, bs):
    pr_resp_len = p_len + r_len
    # Convert to python lists then back to torch
    p = p.tolist()
    r = r.tolist()
    p_len = p_len.tolist()
    r_len = r_len.tolist()
    max_len = max(p_len) + max(r_len)
    pr_resp = []
    for i in range(bs):
        pr_resp.append(p[i][:p_len[i]] + r[i][:r_len[i]] + [0]*(max_len-p_len[i]-r_len[i]))
    pr_resp = torch.LongTensor(pr_resp)
    return pr_resp, pr_resp_len
...
def main(args):
    ...
    hyperparameters = {'VOCAB_SIZE': VOCAB_SIZE, 'EMBD_DIM': EMBD_DIM, 'dropout': args.dropout, 'nhead':args.nhead, 'dim_feedforward': args.dim_feedforward, 'num_encoder_layers': args.num_encoder_layers}
    my_model = SketchyReader(hyperparameters, device)
    ...
    for epoch in range(args.n_epochs):
        ...
        for p_id, r, r_len in train_dl:
            ...
            pr_resp, pr_resp_len = _join_pr_resp(p, p_len, r, r_len, batch_size*2)
            pr_resp, pr_resp_len = pr_resp.to(device), pr_resp_len.to(device)

            # Forward pass
            y_pred = my_model.forward(pr_resp, pr_resp_len, batch_size*2)
    ...
    file_path = args.save_path+'sketchy_seed'+str(args.seed)+'.pt'
